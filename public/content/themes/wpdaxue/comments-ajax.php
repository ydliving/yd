...
if ( empty($status->comment_status) ) {
	...
	err('无效的评论状态'); // 將 exit 改為錯誤提示
} elseif ( !comments_open($comment_post_ID) ) {
	do_action('comment_closed', $comment_post_ID);
	err('抱歉，该文章的评论功能已关闭'); // 將 wp_die 改為錯誤提示
...
if ( $user->ID ) {
	...
	if ( get_option('comment_registration') || 'private' == $status->post_status )
		err('抱歉，你必须登录以后才能发表评论');	// 將 wp_die 改為錯誤提示
}
...
if ( get_option('require_name_email') && !$user->ID ) {
	if ( 6 > strlen($comment_author_email) || '' == $comment_author )
        err( '错误：请填写必填的项目（名字、邮箱）'); // 將 wp_die 改為錯誤提示
    elseif ( !is_email($comment_author_email))
        err('错误：请输入一个有效的邮箱地址'); // 將 wp_die 改為錯誤提示
}

if ( '' == $comment_content )
    err('错误：请输入评论内容'); // 將 wp_die 改為錯誤提示

// 增加: 錯誤提示功能
function err($ErrMsg) {
	header('HTTP/1.1 405 是不允许的方法');
	echo $ErrMsg;
	exit;
}
...
<li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
	<div id="comment-<?php comment_ID(); ?>" class="comment-body">
		<div class="comment-author vcard">
			<img src="<?php echo get_template_directory_uri().'/images/gravatar.png' ?>" alt="" class="avatar" />
			<?php printf( __( '<cite class="fn">%s</cite>'), get_comment_author_link() ); ?>:<?php edit_comment_link( '编辑', ' ' ); ?>
		</div>
